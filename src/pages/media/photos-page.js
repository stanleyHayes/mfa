...
import {selectPhoto} from "../../redux/features/photos/photo-slice";
...
const PhotosPage = () => {
    ...
    const {photos, photosLoading, photosError, count} = useSelector(selectPhoto);
    ...
    return (
        <Layout>
            <Box sx={{py: 8}}>
                {photosLoading && <LinearProgress variant="query" color="secondary"/>}
                <Container sx={{minHeight: '60vh'}}>
                    {photosError && (
                        <Alert severity="error">
                            <AlertTitle>{photosError}</AlertTitle>
                        </Alert>
                    )}
