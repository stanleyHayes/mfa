...
import {selectAnnouncement} from "../../redux/features/announcements/announcement-slice";
...
const AnnouncementsPage = () => {
    ...
    const {announcements, announcementsLoading, announcementsError, count} = useSelector(selectAnnouncement);
    ...
    return (
        <Layout>
            <Box sx={{py: 8}}>
                {announcementsLoading && <LinearProgress variant="query" color="secondary"/>}
                <Container sx={{minHeight: '60vh'}}>
                    {announcementsError && (
                        <Alert severity="error">
                            <AlertTitle>{announcementsError}</AlertTitle>
                        </Alert>
                    )}
                    ...
                    <Grid sx={{mb: 6}} container={true} spacing={4}>
                        {
                            announcements && announcements.map(announcement => {
                                return (
                                    <Grid key={announcement._id} item={true} xs={12} md={6} lg={4}>
